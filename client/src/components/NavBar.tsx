...
export default function NavBar() {
  ...
  return (
    <nav className="fixed top-6 left-1/2 -translate-x-1/2 w-[95%] max-w-6xl z-50">
      <div className="holographic-panel px-8 py-4 flex justify-between items-center transition-all bg-black/60 backdrop-blur-xl border-white/5 shadow-[0_0_40px_rgba(0,0,0,0.8)]">
        
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-lg bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center group-hover:shadow-[0_0_15px_rgba(6,182,212,0.6)] transition-all">
            ...
            return (
              <Link 
                ...
                className={`relative px-4 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] transition-all flex items-center gap-2
                  ${isActive ? 'text-cyan-400' : 'text-cyan-100/40 hover:text-cyan-200'}`}
              >
                ...
                {isActive && (
                  <motion.div 
                    layoutId="activeNav"
                    className="absolute -bottom-1 left-0 right-0 h-[2px] bg-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.8)]"
                  />
                )}
              </Link>
            );
